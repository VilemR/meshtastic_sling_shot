...
**All features can be controlled remotely.** The current version supports the following commands (case insensitive):
 ...
 - **LOC ON/OFF** – Enables or disables the extended "LOC" response, which includes a Google Maps link to the reported location. Responds only to "Range test" module messages broadcast. It remains active for approximately 60 minutes before expiring and automatically disabling itself.
